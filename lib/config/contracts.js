...
ContractsConfig.prototype.compileContracts = function(env) {
  ...
  for (var className in compiled_contracts) {
    ...
    var contractName = className.split(':')[1];

    if (this.is_a_interface(contractName, compiled_contracts)) {
      continue;
    }

    this.all_contracts.push(contractName);
    this.contractDB[contractName] = {
      args: [],
      types: ['file'],
...
ContractsConfig.prototype.configureContractsParameters = function(contractsConfig) {
  for(className in contractsConfig) {
    ...
    console.log(JSON.stringify(contractConfig));
