...
Deploy.prototype.deploy_a_contract = function(env, className, cb) {
    ...
    if (contract.address !== undefined) {
      ...
      if (chainContract != undefined && web3.eth.getCode(chainContract.address) !== "0x") {
        ...
        //console.log("code: " + JSON.stringify(contract.compiled));
        ...
        this.deploy_contract(contractObject, contractParams, function(contractAddress) {
          console.log('deploy_contract: ' + contractAddress);
...
Deploy.prototype.generate_constants_file = function() {
  var result = "(function(angular, undefined) {\n\n\'use strict\';\n\n";
  result += "angular.module('constants', [])\n\n";
  ...
  for(className in this.deployedContracts) {
    ...
    result += ".constant('" + className + "_ADDR', \'" + contractAddress + "\')\n"
    result += ".constant('" + className + "_ABI', " + abi + ")\n"
